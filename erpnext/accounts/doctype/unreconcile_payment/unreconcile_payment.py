...
class UnreconcilePayment(Document):
	...
	def on_submit(self):
		...
		for alloc in self.allocations:
			...
			frappe.db.set_value("Unreconcile Payment Entries", alloc.name, "unlinked", 1)  # pg-port: smallint
	...
	if company and doctype and docname:
		...
		if _dt in ["Sales Invoice", "Purchase Invoice"]:
			...
			res = (
				...
				# pg-port: ANSI grouping (bare selects grouped; they are 1:1
				# with voucher_no) and HAVING on the expression, not the alias
				.groupby(
					ple.account,
					ple.party_type,
					ple.party,
			...
			query = (
				...
				.where(Criterion.all(criteria))
				# pg-port: ANSI grouping, same reasoning as above
				.groupby(
					ple.company,
					ple.account,
					ple.party_type,
...
def get_linked_advances(company, docname):
	...
	return (
		...
		.where(Criterion.all(criteria))
		# pg-port: ANSI grouping + HAVING on the expression, not the alias
		.having(Abs(Sum(adv.amount)) > 0)
		.groupby(adv.company, adv.against_voucher_type, adv.against_voucher_no, adv.currency)
		.run(as_dict=True)
	)
